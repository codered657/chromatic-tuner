...
int closestFreq(float freq) {
    ...
    newDiff = fabsf(freq - noteFreq[0]);
    closestFreq = noteFreq[0];
    
    // While the new fequency difference is less than the smallest, keep searching
    while(newDiff < smallestDiff && i < NUM_OF_NOTES) {
        ...
        closestFreq = noteFreq[i];
        
        // Get next difference
        newDiff = fabsf(freq - noteFreq[i]);

    }

    printf("Cents off: %f\n", centsOff(noteFreq[i - 1], freq));
    return ((i - 1) % 12);
